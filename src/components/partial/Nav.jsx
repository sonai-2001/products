  ...
  return (
    <div className=" md:w-[25vw] w-[12vw] h-screen bg-red-100 p-4 relative">
      <div className={`flex flex-col py-8 gap-3 bg-red-100 md:h-fit  absolute ${open ?"top-0 -right-[70px] w-[100px] h-[100vh] ":"-left-[300px]"} md:static `}>
        ...
        <Link to="/add" className=" px-2  py-1 w-fit border-[2px] border-blue-200  rounded-md">
          Add item
        </Link>
        ...
        <ul>
         {category && category.map((c,i)=>(
           <Link to={`?category=${c}`} key={i} className=" w-full block my-2 flex gap-2 items-center justify-start">
           <span style={{
         ...
         <Link to="/" className=" w-full block my-2 flex gap-2 items-center justify-start">
           <span style={{
            backgroundColor:`${color()}`
